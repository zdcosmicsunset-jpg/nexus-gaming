...
import { ArrowLeft, ArrowUpRight, Command, Cpu, Fan, Zap } from "lucide-react";
import Link from "next/link";

const hardwareGuides = [
  { id: "HW-01", title: "Undervolting RTX Series 30/40", type: "Perfil MSI Afterburner", impact: "Extremo", time: "20 min", icon: <Zap className="w-5 h-5" /> },
  { id: "HW-02", title: "Curvas de Ventilación Óptimas", type: "Configuración Térmica", impact: "Medio", time: "10 min", icon: <Fan className="w-5 h-5" /> },
  { id: "HW-03", title: "RAM EXPO/XMP Tuning", type: "Guía de BIOS", impact: "Alto", time: "15 min", icon: <Cpu className="w-5 h-5" /> }
];
...
const GuideRow = ({ guide, index }: { guide: any, index: number }) => {
  ...
  const refractiveGlow = useMotionTemplate`radial-gradient(400px circle at ${mouseX}px ${mouseY}px, rgba(5, 150, 105, 0.05), transparent 80%)`;
  ...
  return (
    ...
    >
      ...
      <div className="flex items-start gap-6 relative z-10 flex-1">
        <div className="mt-1 text-black/30 group-hover:text-[#059669] transition-colors duration-500">{guide.icon}</div>
        <div>
          <div className="flex items-center gap-3 mb-2">
            <span className="text-[10px] font-mono tracking-widest text-black/40 bg-black/5 px-2 py-1 rounded-md">{guide.id}</span>
            <span className="text-[10px] font-bold uppercase tracking-widest text-[#059669]">{guide.type}</span>
          </div>
      ...
      <div className="flex items-center gap-12 relative z-10 shrink-0">
        ...
        <div className="h-12 w-12 rounded-full border border-black/10 flex items-center justify-center text-black/30 group-hover:bg-[#059669] group-hover:text-white group-hover:border-[#059669] transition-all duration-700 shadow-sm"><ArrowUpRight className="w-5 h-5" strokeWidth={1.5} /></div>
      </div>
    </motion.div>
  );
};
...
export default function HardwarePage() {
  return (
    <main className="min-h-screen bg-[#F7F7F5] text-[#1C1C1C] font-sans selection:bg-[#059669] selection:text-white overflow-hidden pb-32">
      <header className="fixed top-0 inset-x-0 z-50 px-6 md:px-12 py-6 flex justify-between items-center w-full border-b border-black/[0.04] backdrop-blur-md bg-[#F7F7F5]/80">
        <Link href="/" className="font-medium tracking-[0.1em] text-sm uppercase text-black/80 flex items-center gap-3 hover:text-[#059669] transition-colors duration-300 group">
          <div className="h-8 w-8 rounded-full border border-black/10 flex items-center justify-center group-hover:border-[#059669]/30 transition-colors"><ArrowLeft className="w-4 h-4 group-hover:-translate-x-1 transition-transform" /></div>
          Volver al Nexus
        </Link>
        <div className="font-medium tracking-[0.1em] text-sm uppercase text-black/40 flex items-center gap-2"><Command className="w-4 h-4" /> 03 // HARDWARE</div>
      </header>
      ...
      <div className="max-w-[1400px] mx-auto px-6 md:px-12 pt-40">
        <motion.div initial={{ opacity: 0, y: 30 }} animate={{ opacity: 1, y: 0 }} transition={{ duration: 1, ease: [0.16, 1, 0.3, 1] }} className="mb-20">
          <div className="flex items-center gap-4 mb-8">
            ...
            <span className="text-[#059669] font-bold uppercase tracking-widest text-xs">Directorio Activo</span>
          </div>
          <h1 className="text-6xl md:text-[7rem] leading-[0.9] font-light tracking-tight mb-8 text-black/90">
            Hardware <br /><span className="font-serif italic text-black/40">Tuning.</span>
          </h1>
          <p className="text-lg text-black/50 font-light max-w-2xl leading-relaxed">Parámetros milimétricos para controlar temperaturas, estabilizar voltajes y llevar los componentes al límite seguro.</p>
        </motion.div>
        ...
        <div className="w-full border-t border-black/[0.05]">
          ...
          {hardwareGuides.map((guide, index) => <GuideRow key={guide.id} guide={guide} index={index} />)}
        </div>
      </div>
    </main>
  );
